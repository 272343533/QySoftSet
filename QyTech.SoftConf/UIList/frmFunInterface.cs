...
namespace QyTech.SoftConf.UIList
{
    public partial class frmFunInterface : qyfLayoutListWithLeft
    {
        ...
        public frmFunInterface():base(GlobalVaribles.EM_Base, GlobalVaribles.EM_Base, GlobalVaribles.SqConn_Base, Guid.Parse("CCFA9FC6-F4A3-448B-BAF0-1F4507BE5627"), "bsFC_Id in (select bsFC_Id from bsFunConf where bsN_Id in (select bsN_Id from bsNavigation where bsA_Id='" + GlobalVaribles.currAppObj.AppId.ToString()+"'))", "InterfaceName")
        {
            InitializeComponent();
