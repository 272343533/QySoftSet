    ...
    <el-row :gutter="24" >
      <el-col :span="8"><div class="grid-content bg-purple">
        <el-card class="box-card">
          <el-tree :data="treeData" :props="defaultProps"  :default-expand-all=true :highlight-current=true @node-click="clicked"></el-tree>
          </el-card>
        </div>
      </el-col>
    ...
    methods:{
      ...
      addNode(){
          ...
          for(var i=0;i<this.treeData.length;i++){
            if("4126d8b8-610c-4789-be00-f475a70db903"==this.treeData[i].id){
              var a={name: '三级 1-1-1'};
              console.log(a);
              this.treeData[i].children.push(a);
              console.log(this.treeData[i].children[0]);
            }
          }
      },
      onSubmit(){
      ...
      deleteNode(){
        ...
        this.$ajax('post','/api/bsnavigation/delete',{idValue:this.formAlignLeft.bsN_Id},function(message){
        }).then(me.getTree());
      },
      getTree(){
      var root=[];
      var me=this;
      this.$ajax('get','/api/bsnavigation/treedis',{
        ...
        for ( var i = 0; i < tree.length; i++)
        {
            ...
            if (ri.pId == ri.id||ri.pId==null||ri.pId=="null"||ri.pId=="")
            {
                root.push (ri);
            }
          }
